...
function normalizeTipo(raw) {
  ...
  if (s.includes("ruido") || s.includes("noise") || s.includes("sonido"))
    return "Ruido";
...
function unitFromParam(param) {
  ...
  if (p.toLowerCase().includes("oxigen") || p.toLowerCase().includes("od"))
    return "mg/L";
  if (p.toLowerCase().includes("dbo")) return "mg/L";
  if (p.toLowerCase().includes("dqo")) return "mg/L";
  if (
    p.toLowerCase().includes("sst") ||
    p.toLowerCase().includes("tss") ||
    p.toLowerCase().includes("solidos")
  )
    return "mg/L";
  if (p.toLowerCase().includes("nitr")) return "mg/L";
  if (p.toLowerCase().includes("amon")) return "mg/L";
  if (p.toLowerCase().includes("aceite") || p.toLowerCase().includes("grasa"))
    return "mg/L";
  ...
  // Ruido
  if (
    p.toLowerCase().includes("laeq") ||
    p.toLowerCase().includes("lmax") ||
    p.toLowerCase().includes("db")
  )
    return "dB";

  // Suelo
  if (
    p.toLowerCase().includes("materia_organica") ||
    p.toLowerCase().includes("materia organica")
  )
    return "%";
  if (p.toLowerCase().includes("tph") || p.toLowerCase().includes("hidrocar"))
    return "mg/kg";
...
export default function EnvImportExcel({ onImport, loading = false }) {
  const handleFile = async (file) => {
    ...
    const legacyDup = canonRows.some((r) =>
      /^ENV-\d+$/i.test(normalizeStr(r.duplicado_de)),
    );
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const retenida_h =
        raw.retenida_h === "" || raw.retenida_h == null
          ? 48
          : Number(raw.retenida_h);
      ...
      if (!ok) {
        ...
        else if (!(laboratorio === "Interno" || laboratorio === "Externo"))
          motivo = "Laboratorio inválido";
        else if (!Number.isFinite(valorNum))
          motivo = `Valor no numérico (${pv.flag})`;
      ...
      accepted.push({
        ...
        preservante:
          preservanteRaw ||
          (tipo === "Agua" || tipo === "Suelo" ? "Preservante aplicado" : "—"),
